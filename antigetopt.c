...
/* global vars */
char *ago_optarg = NULL; //选项参数
char *ago_optname = NULL; //长选项名称
char ago_optchar = '\0';  //短选项名称
...
/*----------------------------- implementation ------------------------------ */
/* 根据选项，返回ago_optlist[]中相应选项的选项描述、选项要求 */
...
chain_start:/* handle the combined short options*/
	if (chain) {
		if (*chain == '\0')
			chain = NULL;
		else {
	...
	while(*argv) {
		/* The option must start with '-',can be short option or long*/
		if (!endoptions && argv[0][0] == '-' && argv[0][1] != '\0') {
			int amb;

			/* note: ago_lookup also sets ago_optname and islong */
			if ((opt = ago_lookup(list, argv[0], &islong, &amb))
			    == NULL)
				return amb ? AGO_AMBIG : AGO_UNKNOWN;

			/* handle the collapsed(combined?) short options */
